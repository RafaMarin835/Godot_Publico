...
func movimiento():
	if existe_destino: #verificar que se tiene un destino
		...
		if get_slide_collision_count() > 0: #detectar alguna colicion
			generar_nuevo_destino() #si coliciono que cambie direccion

		# Verifica si llegó al destino
		if global_position.distance_to(destino) < 5:
			velocity = Vector2.ZERO
			existe_destino = false
			await get_tree().create_timer(1.5).timeout  # esperamos 1.5 segundos
